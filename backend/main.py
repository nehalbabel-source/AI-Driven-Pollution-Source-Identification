...
if __name__ == "__main__":
    if HAS_FASTAPI:
        print("\n🚀 Starting Delhi AQI API server at http://127.0.0.1:8000")
        print("📖 Interactive docs at http://127.0.0.1:8000/docs\n")
        uvicorn.run("main:app", host="127.0.0.1", port=8000, reload=True)
    else:
        # Preview data shapes without a running server
        print("\n📊 Data preview (install FastAPI to run the server):")
        print(df_clean[["date","location","PM2.5","AQI"]].tail(5))
        print("\n📌 Feature importances:")
        for f in feature_importance_data:
            print(f"  {f['feature']:8s}: {f['importance']:.4f}")
